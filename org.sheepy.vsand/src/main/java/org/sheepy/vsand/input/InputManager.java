...
public final class InputManager implements IAdapter
{
	...
	private void onKeyEvent(KeyEvent event)
	{
		...
		if (event.state == EKeyState.PRESSED)
		{
			switch (event.key)
			{
				case 32 -> application.paused(!application.paused()); // space bar
	...
	{
		final int current = application.brushSize();
		final int brushPlus = Math.min(8, current + 1);
		application.brushSize(brushPlus);
	}
	...
	private void smallerBrush()
	{
		...
		application.brushSize(brushMinus);
	}

	private void halfSpeed()
	{
		final int current = application.speed();
		final int speedPlus = Math.min(32, current * 2);
		application.speed(speedPlus);
	}
	...
	private void doubleSpeed()
	{
		...
		application.speed(speedMinus);
	}

	private void onScrollEvent(ScrollEvent event)
	{
		final var mainMaterial = shiftPressed ? application.secondaryMaterial() : application.mainMaterial();
		...
		if (next != null)
		{
			if (shiftPressed)
			{
				application.secondaryMaterial(next);
			}
			else
			{
				application.mainMaterial(next);
			}
		}
	}
	...
	private static Material findNextUserFriendlyMaterial(List<Material> materials, int index, int direction)
	{
		Material next;
		do
		{
