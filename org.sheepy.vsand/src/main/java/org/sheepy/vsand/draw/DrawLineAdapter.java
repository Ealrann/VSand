...
public final class DrawLineAdapter implements IDrawCommandAdapter<DrawLine>
{
	...
	{
		final Vector2i p1 = new Vector2i(command.x1(), command.y1());
		final Vector2i p2 = new Vector2i(command.x2(), command.y2());
		...
		final int halfSize = command.size() >> 1;
		...
		shapeBuffer.putInt(command.size());

		shapeBuffer.putInt(command.x2());
		shapeBuffer.putInt(command.y2());
